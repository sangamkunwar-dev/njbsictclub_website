...
export const Route = createFileRoute("/events")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Hackathons, workshops, and meetups from the ICT Club of NJBS.",
      },
...
function EventsPage() {
  ...
  const past = events.filter((e) => e.status === "past" || new Date(e.date).getTime() <= now);
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {upcoming.map((e) => {
            ...
            return (
              <Card
                key={e.id}
                className="overflow-hidden border-border/50 hover:border-primary/50 hover:shadow-elegant transition-all p-0 flex flex-col"
              >
      ...
      <section>
        <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
          <div>
            <Badge
              variant="outline"
              className="mb-2 border-accent/40 bg-accent/10 text-accent-foreground"
            >
              Archive
            </Badge>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {past.map((e) => (
            <Card
              key={e.id}
              className="group overflow-hidden border-accent/30 bg-accent/5 p-0 shadow-card transition-all hover:-translate-y-1 hover:border-accent/60 hover:bg-accent/10"
            >
              <div className="relative aspect-[16/9] overflow-hidden bg-muted">
                ...
                <Badge
                  variant="outline"
                  className="absolute left-3 top-3 border-accent/50 bg-background/85 text-accent-foreground backdrop-blur"
                >
                  Past event
                </Badge>
              </div>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Dietary needs, team name, questions…"
            />
          </div>
          ...
          {customFields.map((f) => (
            <div key={f.id}>
              ...
              {f.type === "textarea" ? (
                ...
                <Select
                  value={extra[f.id] ?? ""}
                  onValueChange={(v) => setExtra({ ...extra, [f.id]: v })}
                >
                  <SelectTrigger>
          ...
          {!prefill && (
            <p className="text-[11px] text-muted-foreground text-center">
              ...
              to auto-fill and track your registrations.
            </p>
          )}
        </form>
      </DialogContent>
